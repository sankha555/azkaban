...
json read_config(std::string config_name) {
    std::string path = "tests/configs/" + config_name + ".json";
...
size_t count_neurons(const json& layers) {
    ...
    for (const auto& spec : layers) {
        ...
        if (type == "relu") {
            counts.hidden += spec.at("size").get<size_t>();
        } else if (type == "output") {
            counts.output += spec.at("size").get<size_t>();
        }
    }
    return counts.total();
}
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        std::cout << "Sensitive attribute: " << sensitive_attribute << " (values:";
        ...
        for (const size_t index : examples) {
            ...
            for (const float value : sensitive_attribute_values) {
                ...
                std::cout << "Example " << index << " [attr " << sensitive_attribute
                          << " = " << value << "] ";
