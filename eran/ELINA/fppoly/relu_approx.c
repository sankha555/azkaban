...
void handle_relu_layer(elina_manager_t *man, elina_abstract0_t* element, size_t num_neurons, size_t *predecessors, size_t num_predecessors, bool use_default_heuristics){
	...
	for(i=0; i < num_neurons; i++){
		// printf("in_neuron: %f; ", in_neurons[i]->lb);
		...
		// printf("N%d: %f %f\n", i+1, out_neurons[i]->lexpr->sup_coeff[0], out_neurons[i]->lexpr->sup_cst);
	}

	// printf("\n\n Upper Expression\n");
	// for(int i = 0; i < num_neurons; i++){
	// 	printf("N%d: %f %f\n", i+1, out_neurons[i]->uexpr->sup_coeff[0], out_neurons[i]->uexpr->sup_cst);
	// }
}
