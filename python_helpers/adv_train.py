...
class ConvNN(nn.Module):
    """CNN matching the C++ Model<T> description (CIFAR-10, 3x32x32 input):

        Input(3072)                                 # 3 x 32 x 32
        Conv2D(3, 22, 32, 32, 5, 5, 2, 2, 0, 0)     # -> 22 x 14 x 14 = 4312
        ReLU
    ...
    def __init__(self, input_size, hidden_sizes, num_classes, activation):
        ...
        layers = [
            # Conv2D(3, 22, 32, 32, 5, 5, 2, 2, 0, 0) -> 22 x 14 x 14 = 4312
            nn.Conv2d(input_size[0], 22, kernel_size=5, stride=2, padding=0),
            make_activation(),
            # Conv2D(22, 24, 14, 14, 3, 3, 3, 3, 0, 0) -> 24 x 4 x 4 = 384
...
def main():
    ...
    parser.add_argument('--alpha', type=fraction, default=0, 
                        help='PGD step size')
    ...
    if args.training_method == 'pgd':
        print(f"\nStarting PGD adversarial training ({args.act}, epsilon={args.epsilon:.4f}, alpha={args.alpha:.4f}, steps={args.pgd_steps}, lr={args.lr:.4f})")
    else:
