...
**NOTE:** There may be minor changes in proof times from Table 2 and Table 3, depending on the machine on which the artifact is evaluated. Table 2 and Table 3 in the paper were compiled based on experimental runs on an x86_64 machine running Ubuntu 24.04 LTS with an `AMD EPYC 8434P 48-Core` processor and `1.5-3.1 GHz` CPU frequency.
