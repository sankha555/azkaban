...
void ZKDivRange(int party, IntFp* a, IntFp* b, IntFp* c, int dim){

	IntFp r[dim];
	for(int i = 0; i < dim; i++){
		uint64_t c_clt, r_clt;
		if(party == ALICE){
			uint64_t a_clt = HIGH64(a[i].value);
			uint64_t b_clt = HIGH64(b[i].value);

			c_clt = a_clt << FXPSCALE;
			c_clt = c_clt / b_clt;

			r_clt = (a_clt << FXPSCALE) - b_clt * c_clt;
		}

		c[i] = IntFp(c_clt, ALICE);
		r[i] = IntFp(r_clt, ALICE);
	}
	
	IntFp tmp[dim];
	IntFp TWO_POWER_30 = IntFp(1 << 30, PUBLIC);
	for(int i = 0; i < dim; i++){
		tmp[i] = TWO_POWER_30 + c[i].negate();
	}
	ZKcmpRealVrfyPositive(party, tmp, ZERO_COMP_CONSTANT, tmp, dim);

	ZKcmpRealVrfyPositive(party, r, ZERO_COMP_CONSTANT, tmp, dim);
	for(int i = 0; i < dim; i++){
		tmp[i] = b[i] + r[i].negate();
	}
	ZKcmpRealVrfyPositive(party, tmp, ZERO_COMP_CONSTANT, tmp, dim);
	
	IntFp TWO_POWER_SCALE = IntFp(1 << FXPSCALE, PUBLIC); 
	for(int i = 0; i < dim; i++){
		tmp[i] = TWO_POWER_30 * a[i] + b[i] * c[i].negate();
	}

	batch_reveal_check_zero(tmp, dim);
}
...
void ZKDiv(int party, IntFp *x, IntFp *y, int dim, int mode = 0)  //y=1/x
{
	...
	if (mode == -1){
		LUTdiv_mode = LUTdivLower;
	} else if (mode == 1){
		LUTdiv_mode = LUTdivUpper;
	}
...
void ZKminmax4(int party, IntFp* x, IntFp* y){
    // minmax(a, b, c, d) = minmax(minmax(a, b), minmax(c, d))
    
    const int n = 4;
    const int logn = ceil(log(n));

    IntFp* diff = new IntFp[logn];
    IntFp* cmp = new IntFp[logn];

    diff[0] = x[0] + x[1].negate();
    diff[1] = x[2] + x[3].negate();
    ZKcmpPositive(party, diff, (PR+1)/2, cmp, 2);

    IntFp min0, min1, max0, max1;
    max0 = cmp[0]*diff[0] + x[1];
    min0 = x[0] + x[1] + max0.negate();
    
    max1 = cmp[1]*diff[1] + x[3];
    min1 = x[2] + x[3] + max1.negate();

    diff[0] = max0 + max1.negate();
    diff[1] = min0 + min1.negate();
    ZKcmpPositive(party, diff, (PR+1)/2, cmp, 2);
...
// M0 = c0 * d0 + x1        # max(x0, x1)
// m0 = x0 + x1 - M0        # min(x0, x1)

// M1 = c1 * d1 + x3        # max(x2, x3)
// m1 = x2 + x3 - M1        # min(x2, x3)
...
// y0 = c3 * (-d3) + m0     # min(m0, m1)
// y1 = c2 * d2 + M1        # max(M0, M1)