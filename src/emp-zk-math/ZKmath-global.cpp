...
LUTTwoValueIntFp *LUTdivLower;
LUTTwoValueIntFp *LUTdivUpper;
...
void startComputation(int party)
{
    ...
    LUTdivLower = new LUTTwoValueIntFp(party);
    LUTdivUpper = new LUTTwoValueIntFp(party);
    ...
    // Div underapproximate
    for (int i = 0; i < k; i++){
        double p = 1.0 + (double(i) / double(k));
        // For a lower bound starting at p:
        // A0 should be 1/p
        A0 = 1.0 / p;
        ...
        A1 = 1.0 / (p * p);
        val = (uint64_t)ceil(A1 * (1ULL << SCALE)); // Subtract more to stay below
        coff2.push_back(val);
    }
    LUTdivLower->LUTTwoValueinit(coff1, coff2);
    coff1.resize(0);
    coff2.resize(0);

    // Div overapproximate
    for (int i = 0; i < k; i++) {
        // 1. Define interval [p, p_next]
        double p = 1.0 + (double(i) / double(k));
        // double p_next = p + (1.0 / double(k));
        double p_next = p + (1.0 / double(k)) + 1;
        ...
        coff1.push_back(A0_final);
        coff2.push_back(A1_final);
    }
    LUTdivUpper->LUTTwoValueinit(coff1, coff2);
    coff1.resize(0);
    coff2.resize(0);
