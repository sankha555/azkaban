...
// ---------------------------------------------------------------------------
// This is the src/interval/interval.h implementation extended with a dedicated
// `Fp` code path.  The Fp path mirrors the int64_t fixed-point reference but
// (a) reduces every value into the prime field p = 2^61-1 and (b) checks each
// field-relevant intermediate for overflow.  The important checks are on the
// UNTRUNCATED products / accumulators (operator*, operator/, inner_product),
// because those are the values that actually live in the field before the
// truncation gadget runs; if they leave [-(p-1)/2, (p-1)/2] the field wraps and
// the subsequent truncation produces garbage.
// ---------------------------------------------------------------------------
...
class Interval {
    ...
    Interval() {
        if constexpr (TYPE_EQ(T, float) || TYPE_EQ(T, int64_t) || TYPE_EQ(T, Fp)){
            this->inf = T(0);
            this->sup = T(0);
        } else {
            this->inf = FIELD_ZERO;
    ...
    Interval<T> operator-(const Interval<T>& other) const {
        ...
        if constexpr (TYPE_EQ(T, int64_t) || TYPE_EQ(T, Fp)) {
            res.inf = this->inf - other.sup;
            res.sup = this->sup - other.inf;
        } else
    ...
    Interval<T> operator*(const Interval<T>& other) const {
        ...
        if constexpr (TYPE_EQ(T, Fp)){
            ...
            // The field holds the UNTRUNCATED product; if it does not fit the
            // field the following truncation is corrupt -> that is the overflow.
    ...
    static Interval<T> intervalize_from_real(float real){
        ...
        if constexpr (TYPE_EQ(T, Fp)) {
            ...
            interval = Interval<Fp>(Fp(l), Fp(u));
        }
