...
#include "src_fp/params.h"

// Active Conv2D (mirrors the enabled class in src/conv2d.h). Fully generic:
// it only uses Interval<T>/Zonotope<T>/inner_product, so the Fp overflow checks
// happen automatically inside Interval<Fp>.
template <typename T>
